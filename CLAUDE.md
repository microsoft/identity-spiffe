# CLAUDE.md — AIM Prototype Platform
...
- Use `./deploy.sh --portal-only` only for portal or CrowdStrike-only changes. Do not use it for sidecar, SPIRE, agent, or Entra bootstrap changes.
...
- `aim-portal` and `crowdstrike-mock` are separate Container Apps with Entra browser sign-in.
...
- Portal auth groups are shared tenant-wide: `AIM Administrators` and `AIM Viewers`.

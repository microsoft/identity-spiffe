#!/bin/bash
# =============================================================================
# AIM Prototype Platform - SPIFFE/SPIRE Deployment (Join Token Attestation)
# =============================================================================
...
build_portal_images() {
    ...
    echo "   Building crowdstrike-mock:${IMAGE_TAG} (cache-bust=${cache_bust_val})..."
    az acr build \
        --registry "$ACR_NAME" \
        --image "crowdstrike-mock:${IMAGE_TAG}" \
        --file crowdstrike-mock/Dockerfile \
        --build-arg "CACHE_BUST=${cache_bust_val}" \
        --build-arg "BUILD_VERSION=${IMAGE_TAG}" \
        .
...
if [ -n "$AZD_ENV_NAME" ] && [[ ! "$AZD_ENV_NAME" =~ ^aim- ]]; then
    ...
    echo "     azd env new aim-foundry-poc"
    echo "     ./deploy.sh"
    echo ""
    echo "   Or rename the current environment:"
    echo "     azd env new aim-foundry-poc"
    echo "     azd env select aim-foundry-poc"
    echo "     ./deploy.sh"
    exit 1
fi
...
# =============================================================================
# Step 0: Detect existing AIM environments in the subscription
# =============================================================================
...
detect_existing_environment() {
    echo "🔍 Checking for existing AIM environments in subscription..."
    local sub_name
    ...
    if [ -z "$existing_rgs" ]; then
        echo "   No existing AIM environments found (no resource groups matching '${AIM_RG_PREFIX}*')"
        echo ""
        return 1
    fi

    echo ""
    echo "   Found existing AIM environment(s):"
    echo "   ──────────────────────────────────────────────"
    local rg_count=0
    local rg_names=()
...
echo ""
echo "============================================="
echo "  AIM Prototype Platform - SPIFFE mTLS PoC"
...
if [ "$SKIP_PROVISION" = true ]; then
    ...
    if [ "$GITHUB_AGENT" = true ]; then
        ...
        azd env set GITHUB_REPO "${GITHUB_REPO:-aim-foundry-poc}"
    fi
...
# Portal and CrowdStrike mock need ReadWrite; sidecar + agents need Read.
...
_EARLY_CS_URL=$(azd_env_get_from_blob "$AZD_VALUES" "SERVICE_CROWDSTRIKE_MOCK_ENDPOINT_URL")
if [ -n "$_EARLY_PORTAL_URL" ]; then
    echo ""
    echo "   Portal:          ${_EARLY_PORTAL_URL}"
    echo "   CrowdStrike Mock: ${_EARLY_CS_URL}"
fi
echo ""
...
if [ "$SKIP_BUILD" = true ]; then
    ...
    # Build portal and CrowdStrike mock images
...
PORTAL_CROWDSTRIKE_APP_NAME=$(portal_crowdstrike_app_display_name)
...
# Add current user to AIM Administrators (best-effort)
...
if [ -n "$CURRENT_USER_OID" ]; then
    ...
    echo "   Current user added to AIM Administrators"
fi
...
CS_MOCK_FQDN=$(azd_env_get_from_blob "$AZD_VALUES" "SERVICE_CROWDSTRIKE_MOCK_ENDPOINT_URL")
CS_MOCK_FQDN="${CS_MOCK_FQDN:-}"
...
# Create CrowdStrike mock app registration
CS_AUTH_CLIENT_ID=$(azd_env_get_from_blob "$AZD_VALUES" "CROWDSTRIKE_AUTH_CLIENT_ID")
CS_APP_RESULT=$(ensure_portal_app "$CS_AUTH_CLIENT_ID" "$PORTAL_CROWDSTRIKE_APP_NAME" "$CS_MOCK_FQDN" "http://localhost:8560/")
CS_AUTH_CLIENT_ID=$(printf '%s' "$CS_APP_RESULT" | cut -f1)
CS_APP_OBJ_ID=$(printf '%s' "$CS_APP_RESULT" | cut -f2)
if [ -n "$CS_AUTH_CLIENT_ID" ]; then
    azd env set CROWDSTRIKE_AUTH_CLIENT_ID "$CS_AUTH_CLIENT_ID"
    echo "   ✅ CrowdStrike mock app: ${CS_AUTH_CLIENT_ID}"
else
    echo "   ⚠️  CrowdStrike mock app registration failed — auth will be disabled"
fi
...
if [ -n "$CS_AUTH_CLIENT_ID" ]; then
    echo "   Granting admin consent for CrowdStrike mock app..."
    az ad app permission admin-consent --id "$CS_AUTH_CLIENT_ID" 2>/dev/null || true
fi
...
PORTAL_MI_CLIENT_ID=$(az containerapp show --name aim-portal --resource-group "$RG" \
    --query "identity.userAssignedIdentities.*.clientId | [0]" -o tsv 2>/dev/null || true)
CS_MOCK_MI_CLIENT_ID=$(az containerapp show --name crowdstrike-mock --resource-group "$RG" \
    --query "identity.userAssignedIdentities.*.clientId | [0]" -o tsv 2>/dev/null || true)
...
if [ -n "$PORTAL_AUTH_CLIENT_ID" ] || [ -n "$ADMIN_CP_URL" ]; then
    ...
    if [ -z "$CS_MOCK_MI_CLIENT_ID" ]; then
        echo "ERROR: crowdstrike-mock managed identity client ID could not be resolved." >&2
        exit 1
    fi

    portal_secret_args=("mgmt-api-key=${MGMT_KEY}")
    crowdstrike_secret_args=("mgmt-api-key=${MGMT_KEY}")
    ...
    crowdstrike_env_vars=(
        "ADMIN_CP_URL=${ADMIN_CP_URL}"
        "MGMT_API_KEY=secretref:mgmt-api-key"
        "AZURE_TENANT_ID=${AZURE_TENANT}"
        "AZURE_CLIENT_ID=${CS_MOCK_MI_CLIENT_ID}"
        "AUTH_CLIENT_ID=${CS_AUTH_CLIENT_ID}"
        "AIM_ADMIN_GROUP_ID=${AIM_ADMIN_GROUP_ID}"
        "AIM_VIEWER_GROUP_ID=${AIM_VIEWER_GROUP_ID}"
        "PORTAL_MODE=cloud"
    )
    if [ -n "$GRAPH_CLIENT_ID_VAL" ]; then
        ...
        crowdstrike_secret_args+=("graph-client-id=${GRAPH_CLIENT_ID_VAL}" "graph-client-secret=${GRAPH_CLIENT_SECRET_VAL}")
        portal_env_vars+=("GRAPH_CLIENT_ID=secretref:graph-client-id" "GRAPH_CLIENT_SECRET=secretref:graph-client-secret")
        crowdstrike_env_vars+=("GRAPH_CLIENT_ID=secretref:graph-client-id" "GRAPH_CLIENT_SECRET=secretref:graph-client-secret")
    else
    ...
    # Update AIM Portal container app
    ...
    run_az_step "Failed to set crowdstrike-mock secrets" \
        az containerapp secret set \
        --name crowdstrike-mock \
        --resource-group "$RG" \
        --secrets "${crowdstrike_secret_args[@]}"

    # Update CrowdStrike mock container app
    echo "   Updating crowdstrike-mock container app..."
    run_az_step "Failed to update crowdstrike-mock container app" \
        az containerapp update \
        --name crowdstrike-mock \
        --resource-group "$RG" \
        --image "${ACR_SERVER}/crowdstrike-mock:${IMAGE_TAG}" \
        --set-env-vars "${crowdstrike_env_vars[@]}"
...
echo "  PUT  /agent-risk  - Update agent risk (mock CrowdStrike)"
...
echo "║  CrowdStrike Mock:  ${CS_MOCK_FQDN:-<not deployed>}"
echo "║                                                                   ║"
echo "╚═══════════════════════════════════════════════════════════════════╝"
echo ""
...
if [ "$GOOGLE_AGENT" = true ]; then
    ...
    curl -s -X PUT \
        -H "X-AIM-Admin-Key: $MGMT_API_KEY" \
    ...
    CURRENT_IDS=$(curl -s -H "X-AIM-Admin-Key: $MGMT_API_KEY" \
...
if [ "$GITHUB_AGENT" = true ]; then
    ...
    GITHUB_REPO="${GITHUB_REPO:-aim-foundry-poc}"

#!/usr/bin/env bash
# =============================================================================
# AIM Prototype Platform — Provision GCE VM for Cross-Cloud Agent
# =============================================================================
...
echo ""
echo "============================================================"
echo "  AIM — Provision GCE VM for Cross-Cloud Agent"
echo "============================================================"
echo ""
...
if gcloud iam service-accounts describe "$SA_EMAIL" --project="$PROJECT" &>/dev/null; then
    ...
    gcloud iam service-accounts create "$GCP_SA_NAME" \
        --project="$PROJECT" \
        --display-name="AIM cross-cloud agent identity" \
        --quiet
...
if gcloud compute firewall-rules describe "aim-allow-ssh" --project="$PROJECT" &>/dev/null; then
    ...
    gcloud compute firewall-rules create "aim-allow-ssh" \
        ...
        --description="Allow SSH access to AIM agents" \
        --quiet
...
if gcloud compute firewall-rules describe "aim-allow-https" --project="$PROJECT" &>/dev/null; then
    ...
    gcloud compute firewall-rules create "aim-allow-https" \
        ...
        --source-ranges="0.0.0.0/0" \
        --description="Allow HTTPS access to AIM agents (443 + 8443 for invoke_url)" \
        --quiet
...
if gcloud compute firewall-rules describe "aim-allow-agent-http" --project="$PROJECT" &>/dev/null; then
    ...
    gcloud compute firewall-rules create "aim-allow-agent-http" \
        ...
        --description="Allow HTTP access to AIM agent endpoint (port 8000) from Azure VPN CIDR" \
        --quiet
...
echo "    3. Register with AIM portal:"

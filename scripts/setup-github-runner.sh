#!/bin/bash
# =============================================================================
# AIM — Setup GitHub Actions Self-Hosted Runner with SPIRE + spiffe-proxy
# =============================================================================
...
echo ""
echo "============================================================"
echo "  AIM — GitHub Runner Setup: ${AGENT_NAME}"
echo "============================================================"
...
if [[ ! -f ".runner" ]]; then
    ...
    su - "$RUNNER_USER" -c "cd $RUNNER_DIR && ./config.sh \
        ...
        --name \"aim-runner-\$(hostname)\" \
        --labels \"self-hosted,aim-runner,Linux,X64\" \
        --runnergroup \"Default\" \
        --work \"_work\" \
        --unattended \
        --replace"
...
[Unit]
Description=AIM spiffe-proxy (egress — to ${BUDGET_BACKEND_FQDN:-<unset>})
...
cat > /etc/aim-github-runner.env << ENVFILE
# AIM GitHub Runner environment — consumed by workflow steps via envfile
...
ENVFILE
chmod 644 /etc/aim-github-runner.env

echo "  ✅ Environment config written to /etc/aim-github-runner.env"
...
echo "  Env file:           /etc/aim-github-runner.env"
echo ""
echo "  Check services:"

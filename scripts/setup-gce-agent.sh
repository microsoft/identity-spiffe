#!/usr/bin/env bash
# =============================================================================
# setup-gce-agent.sh — Master GCE VM Setup for Cross-Cloud AIM Agent
# =============================================================================
...
server {
    ...
    ca_subject {
        ...
        common_name = "Project AIM GCP SPIRE CA"
    }
...
async def call_backend_raw(request: _Request, method: str = "GET", path: str = "/budget/read", body: str = ""):
    ...
    if not _secrets.compare_digest(request.headers.get("X-AIM-Admin-Key", ""), _ADMIN_KEY):
...
[Unit]
Description=AIM Google Budget Reader Agent (FastAPI)

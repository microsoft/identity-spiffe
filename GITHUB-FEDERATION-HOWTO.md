...
Adds the GitHub runner to an existing Identity Research for Agent Management Using SPIFFE deployment.
...
```bash
gh workflow run "Identity Research for Agent Management Using SPIFFE Budget Check" --repo microsoft/identity-spiffe
```

Or from the GitHub Actions UI: go to Actions → Identity Research for Agent Management Using SPIFFE Budget Check → Run workflow.
...
Azure limits FICs to 20 per app registration. Identity Research for Agent Management Using SPIFFE solves this:

- **One Flexible FIC** on the Blueprint trusts `repo:microsoft/*`
- **RBAC `required_tags`** authorize per-repo (e.g., `github_repo: microsoft/identity-spiffe`)
...
```bash
gh api repos/microsoft/identity-spiffe/actions/runners --jq '.runners[] | {name, status}'
```

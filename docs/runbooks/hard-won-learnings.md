...
**Rule:** NEVER use `azd deploy` for agent Container Apps. Only safe for portals (`aim-portal`, `crowdstrike-mock`) which have no SPIFFE sidecar.
...
**Correct:**
```bash
curl -X PUT -H "Content-Type: application/x-yaml" -H "X-AIM-Admin-Key: $KEY" \

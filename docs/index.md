# AIM Prototype Platform Documentation

Project AIM demonstrates four-layer authorization for agent-to-agent traffic on Azure Container Apps using Microsoft Entra Agent Identity, SPIFFE/SPIRE, sidecar enforcement, and admin governance.

## What This Repo Proves

The platform combines four independent enforcement layers:
...
| Layer 4 | Conditional Access and admin governance | Whether the organization currently allows the caller to operate |

Those layers are intentionally independent. A caller that clears RBAC can still fail token validation. A caller with a valid token can still be blocked by Conditional Access. A caller blocked by mTLS never reaches the later layers.
...
| `crowdstrike-mock` | Mock SOC portal that pushes risk signals into AIM |

...
- `repository`: full repo name (e.g., `microsoft/aim-foundry-poc`)
...
AIM uses a **single Flexible FIC** on the Agent Identity Blueprint with `claimsMatchingExpression`:
...
This trusts ALL repos in the `microsoft` org with ONE FIC. Per-repo authorization is handled by AIM's RBAC engine (`required_tags` on rules), not by FIC proliferation.
...
- GitHub Actions runner: `aim-runner-github`, online, labels: `self-hosted,aim-runner,Linux,X64`
...
```bash
gh workflow run "AIM Budget Check"
```

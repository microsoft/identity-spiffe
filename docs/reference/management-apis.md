...
|---|---|---|
| `/health` | `GET` | none |
| `/admin/agents` | `GET` | `X-AIM-Admin-Key` |
| `/admin/{mgmt_path}` | `GET`, `PUT` | `X-AIM-Admin-Key` |
...
All of those routes are guarded by `X-AIM-Admin-Key`.

## Contract Guidance

- Public browsers should only talk to the portal or CrowdStrike mock.

"""
Mock CrowdStrike Portal — Agent Risk Signal Source
=====================================================
Simulates an external threat detection service (CrowdStrike, Microsoft Defender,
etc.) that pushes agent risk levels to the AIM sidecar's management API.
...
Usage:
    python3 crowdstrike-mock/server.py
...
Optional overrides:
    python3 crowdstrike-mock/server.py --port 8560 --config path/to/portal-config.json
    python3 crowdstrike-mock/server.py --mgmt-url https://admin-control-plane.xyz.azurecontainerapps.io/admin
"""
import json
import os
import copy
...
logger = logging.getLogger("crowdstrike-mock")
...
app = FastAPI(
    title="Mock CrowdStrike — Agent Risk Portal",
    description="External threat signal source for AIM agent risk enforcement",
    version="3.0.0",
)
...
def _mgmt_headers(extra=None):  # type: (Optional[Dict[str, str]]) -> Dict[str, str]
    """Build headers for admin-control-plane requests (X-AIM-Admin-Key)."""
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Mock CrowdStrike Portal")
    ...
    logger.info("Starting Mock CrowdStrike Portal: http://localhost:%d", args.port)

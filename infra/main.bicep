...
param githubRepo string = 'aim-foundry-poc'
...
param crowdstrikeMockImage string = 'mcr.microsoft.com/k8se/quickstart:latest'

// Auth configuration (populated by deploy.sh after Entra app registration)
param portalAuthClientId string = ''
param crowdstrikeAuthClientId string = ''
...
module crowdstrikeMock 'modules/portal-app.bicep' = {
  name: 'crowdstrike-mock'
  scope: rg
  params: {
    name: 'crowdstrike-mock'
    location: location
    tags: union(tags, { 'azd-service-name': 'crowdstrike-mock' })
    environmentId: containerAppsEnv.outputs.environmentId
    registryServer: acr.outputs.loginServer
    acrResourceId: acr.outputs.id
    containerImage: crowdstrikeMockImage
    targetPort: 8560
    secrets: [
      { name: 'mgmt-api-key', value: mgmtApiKey }
      { name: 'graph-client-id', value: graphClientId }
    ...
    envVars: [
      ...
      { name: 'MGMT_API_KEY', secretRef: 'mgmt-api-key' }
      { name: 'AZURE_TENANT_ID', value: azureTenantId }
      { name: 'AUTH_CLIENT_ID', value: crowdstrikeAuthClientId }
      { name: 'AIM_ADMIN_GROUP_ID', value: aimAdminGroupId }
      { name: 'AIM_VIEWER_GROUP_ID', value: aimViewerGroupId }
      { name: 'GRAPH_CLIENT_ID', secretRef: 'graph-client-id' }
      { name: 'GRAPH_CLIENT_SECRET', secretRef: 'graph-client-secret' }
...
output SERVICE_CROWDSTRIKE_MOCK_ENDPOINT_URL string = 'https://${crowdstrikeMock.outputs.fqdn}'

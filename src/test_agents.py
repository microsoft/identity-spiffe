...
class TestBudgetReport(unittest.TestCase):
    ...
    def test_call_backend_raw_rejects_bad_method(self):
        ...
        resp = client.post(
            ...
            headers={"X-Spiffe-Admin-Key": "test-raw-key"},
        )
        data = resp.json()
    ...
    def test_flush_token_requires_admin_key(self):
        ...
        # Wrong key → 401
        resp = client.post("/flush-token", headers={"X-Spiffe-Admin-Key": "wrong"})
        self.assertEqual(resp.status_code, 401)
        # Correct key → 200
        resp = client.post("/flush-token", headers={"X-Spiffe-Admin-Key": "test-key-123"})
        self.assertEqual(resp.status_code, 200)
        os.environ.pop("MGMT_API_KEY", None)
...
class TestBudgetApproval(unittest.TestCase):
    ...
    def test_flush_token_requires_admin_key(self):
        ...
        resp = client.post("/flush-token")
        self.assertEqual(resp.status_code, 401)
        resp = client.post("/flush-token", headers={"X-Spiffe-Admin-Key": "test-key-456"})
        self.assertEqual(resp.status_code, 200)
        os.environ.pop("MGMT_API_KEY", None)

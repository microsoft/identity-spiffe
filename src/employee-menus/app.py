"""
EmployeeMenus - Blocked Caller Agent
======================================
Has no network trust to BudgetBackend. Blocked at mTLS handshake.
Maps to slide 50 where EmployeeMenus has "?" for Budget Backend role.
"""
import os
import re
import logging
...
logger = logging.getLogger(os.getenv("AGENT_NAME", "employee-menus"))

app = FastAPI(
    title=f"Agent {os.getenv('AGENT_NAME', 'employee-menus')}",
    description="AIM Prototype Platform: EmployeeMenus — blocked caller (no network trust)",
    version="0.2.0",
)
...
async def health():
    return {
        "status": "healthy",
        "agent": os.getenv("AGENT_NAME", "employee-menus"),
...
async def call_backend(action: str = "echo", message: str = "hello from caller"):
    agent_name = os.getenv("AGENT_NAME", "employee-menus")
...
async def call_backend_raw(request: Request, method: str = "GET", path: str = "/budget/read", body: str = ""):
    ...
    if not ADMIN_KEY:
        return JSONResponse({"error": "mgmt_api_key_not_configured"}, status_code=500)
    if not secrets.compare_digest(request.headers.get("X-AIM-Admin-Key", ""), ADMIN_KEY):
        return JSONResponse(
            {
                "caller": agent_name,
                "target": "budget-backend",
    ...
    if not request_body and method.upper() in ("POST", "PUT", "PATCH"):
        request_body = '{"amount": 5000, "description": "RBAC test from EmployeeMenus"}'
...
async def a2a_status(request: Request):
    ...
    return {
        ...
        "agent": agent_name,
        "caller": caller_id,
        "resource": "employee-menus",
        "summary": "Direct A2A status from EmployeeMenus",
        "enforcement": {
            "jwt_validated": True,
            "jwt_oid": jwt_claims["oid"],
            "jwt_roles": jwt_claims["roles"],

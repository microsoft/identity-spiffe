"""
Demo Agent — Generic Caller for Dynamic Onboarding
====================================================
Generic caller agent used by scripts/add-demo-agent.sh. The AGENT_NAME env var
(set per Container App) controls identity. Same code, different SPIFFE ID.

Supports the full A2A contract: /call-agent (caller), /a2a/status (target),
JWT validation, risk/tag queries, and dynamic MI/target discovery.
"""
import os
import re
import logging
...
logger = logging.getLogger(os.getenv("AGENT_NAME", "demo-agent"))

app = FastAPI(
    title=f"Agent {os.getenv('AGENT_NAME', 'demo-agent')}",
    description="AIM Prototype Platform: dynamically onboarded caller agent",
    version="0.3.0",
)

BACKEND_ENDPOINT = os.getenv("BACKEND_ENDPOINT", os.getenv("A2_RESOURCE_ENDPOINT", "http://localhost:8080"))
...
async def health():
    return {
        "status": "healthy",
        "agent": os.getenv("AGENT_NAME", "demo-agent"),
...
async def flush_token(request: Request):
    ...
    if request.headers.get("X-AIM-Admin-Key") != ADMIN_KEY:
        return JSONResponse({"error": "unauthorized"}, status_code=401)
    flush_cached_token()
    return {"status": "flushed", "message": "Token cache cleared."}


@app.post("/call-backend")
async def call_backend(action: str = "echo", message: str = "hello from caller"):
    agent_name = os.getenv("AGENT_NAME", "demo-agent")
...
async def call_backend_raw(method: str = "GET", path: str = "/budget/read", body: str = ""):
    ...
    if not request_body and method.upper() in ("POST", "PUT", "PATCH"):
        request_body = '{"amount": 5000, "description": "RBAC test from demo agent"}'
...
async def a2a_status(request: Request):
    ...
    jwt_claims = validate_entra_jwt(auth_header[7:])
    ...
    return {
        ...
        "caller": caller_id,
        "resource": agent_name,
        "summary": f"Direct A2A status from {agent_name}",
        "enforcement": {
            "jwt_validated": True,
            "jwt_oid": jwt_claims["oid"],
            "jwt_roles": jwt_claims["roles"],

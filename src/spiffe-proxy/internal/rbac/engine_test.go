  ...
  - spiffe_id: "spiffe://aim.microsoft.com/agent/github-budget-reader"
    ...
    rules:
      - path: "/budget/read"
        ...
        required_roles: ["Budget.Read"]
        required_tags:
          github_repo: "microsoft/aim-foundry-poc"
      - path: "/budget/any"
        methods: ["GET"]
        action: allow
        require_jwt: true
	...
	mockVal := &mockValidator{
		claims: &oauth.Claims{
			...
			CustomClaims: map[string]string{
				"github_repo":         "microsoft/aim-foundry-poc",
				"github_workflow_ref": "microsoft/aim-foundry-poc/.github/workflows/deploy.yml@refs/heads/main",
			},
		},
	}
...
func TestTag_MatchingClaims_Allowed(t *testing.T) {
	...
	if d.CustomClaims["github_repo"] != "microsoft/aim-foundry-poc" {
